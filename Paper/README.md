...
[Dentro de los algoritmos de optimización tenemos... 
Mientras que, por otro lado, los modelos de optimización métodos tradicionales como nonlinear programming (NLP) techniques basadas en gradientes (*CITAS*), por ejemplo, the steepest descent method, conjugate gradient method, Gauss–Newton method, etc; y métodos no tradicionales, como los algoritmos evolutivos (*CITAS*), por ejemplo, (*****). La desventaja de los algorítmos basados en gradientes es que pueden dar como resultado valores óptimos locales en lugar de globales debido a la falta de convexidad inherente en los modelos de acuíferos, ya que la ecuación del modelo de flujo puede ser lineal con respecto a una variable de estado, que a su vez es altamente no lineal con respecto a los parámetros del sistema \citep{LakshmiPrasadAndRastogi2001, Batenietal2015}. Por tanto, para abordar estos inconvenientes (*shortcomings*) se aplican los algoritmos evolutivos, ya que pueden manejar problemas altamente no lineales y convergen al óptimo global en lugar del local \citep{Batenietal2015}. 

[A pesar de su potente aplicación en problemas físicos (CITAS)... El uso de algoritmos en problemas pesados** puede deteriorar su eficiencia (CITA COMO EN DE) por tanto las metodologías distribuidas que hacen uso de simulaciones en paralelo* resultan eficaces. Investigaciones como ... (CITAS - Considerar hablar de varios algoritmos distribuido, no solo DE y PSO) implementan los algoritmos de forma distribuida... Relacionado a las agua subterráneas muchos logran simplificar los procesos con usos de modelos de simulación más livianos (MFree) sin embargo, como se comentó, analizar la interacción con el sistema superficial resulta relevante, Por eso esta investigación aborda implementar metodología híbrida y asíncrona de algoritmos para no perder el potente avance de los modelos físicos --> REORDENAR IDEA.
...
La simulación del sistema subterráneo se realiza utilizando modelos basado en cuadrículas o mallas, como los modelos de diferencias finitas (FDM) o de elementos finitos (FEM); o simuladores independientes de una malla, como Meshfree (Mfree). 
...
>>>>>>> 4def719b76f119fa72b0a82d7d21398463adc434
