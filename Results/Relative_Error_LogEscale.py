...
for n in variables:
    ...
    plt.colorbar(im)
    plt.text(165, 5, 'MAPE: ' + str(round(MAPE_e,2)) + '%', fontsize = 10)
    #plt.colorbar(fraction=0.047*im_ratio, extend='max')
    plt.clim(0.001, 10000)

    if n == 'kx':
        plt.title(methodology + ' - Relative error (%) - Hydraulic Conductivity')
    else:
        plt.title(methodology + ' - Relative error (%) - Specific yield')
    plt.xlabel('Column')
    plt.ylabel('Row')
    
    plt.savefig(os.path.join(r'D:\1_PaperI', methodology, 'Graficas', 'Error_relativo_' + n + '_' + methodology + '_best_experiment.png'))
    #plt.show()
    plt.clf()
