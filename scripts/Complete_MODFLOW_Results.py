...
ruta = dir
print(ruta)
ruta_WEAP = 'C:/Users/Francisco Suárez P/Documents/WEAP Areas/Ligua_Petorca_WEAP_MODFLOW_RDM'
...
if __name__ == "__main__":
    ...
    ruta_BALANCE_ZB = dir + '/' + version + '/' + ZB[0][0:-4]
    ruta_BALANCE_ZB_RL = dir + '/' + version + '/' + ZB[1][0:-4]

    ruta_export_BALANCE = dir + '/' + version + '/BALANCE'
    if not os.path.isdir(ruta_export_BALANCE):
        os.mkdir(ruta_export_BALANCE)

    fecha = pd.read_csv(dir + '/Fechas.csv')
    años = pd.read_csv(dir + '/Años.csv')
